  ...
  error: {
    ...
    icon: (
      <svg
        ...
        className='currentColor size-3.5'
      >
  ...
  success: {
    ...
    icon: (
      <svg
        ...
        className='currentColor size-3.5'
      >
        <path d='M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1Zm-1 9.795-2.5-2.5.795-.795L7 9.205 10.705 5.5l.798.793L7 10.795Z' />
      </svg>
    ),
    label: 'Success',
  },
...
export function Callout({ type = 'info', title, children }: CalloutProps) {
  ...
  return (
    <aside
      className={`my-4 rounded-md border py-4 pr-4 pl-4 ${config.className}`}
      data-callout={type}
      aria-label={config.label}
    >
      <div className='flex gap-3'>
        <span
          className='shrink-0 pt-[3px] text-lg leading-none text-(--callout-accent)'
          aria-hidden
        >
          {config.icon}
        </span>
        <div className='min-w-0 flex-1 space-y-1 [&>p:first-child]:mt-0 [&>p:last-child]:mb-0'>
          {title !== null && title !== '' && (
            <p className='mb-1 text-sm font-semibold text-(--callout-accent)'>
              {title}
            </p>
          )}
