...
The `as const` assertion is necessary for proper type inference because Next.js generates explicit types for parallel route slots at build time. Next.js typegen creates a `LayoutSlotMap` interface in `.next/types/routes.d.ts` that maps each route to its slot names as a union type (e.g., `"/playground/parallel": "analytics" | "team"`).

The `LayoutProps` from `createSafeLayoutServerComponent` needs to stick to the Next.js typing requirements for build time.

Without `as const`, TypeScript will infer the option `experimental_slots` as an index signature like `{[x: string]: React.ReactNode}` instead of an explicit signature as expected by Next.js This prevents the type system from matching the exact slot names that Next.js expects, breaking type safety, and build.
