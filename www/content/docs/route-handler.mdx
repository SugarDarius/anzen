...
Callback triggered when the request fails. By default it returns a simple `500` response and the error is logged into the console. Use it if your handler use custom errors and you want to manage them properly by returning a proper response. You can read more about it under the [Error handling](#error-handling) section.
